...
```json
{"ts": "2026-06-25T13:03:59.774911+00:00", "method": "POST", "path": "/mcp", "status": 200, "ms": 3, "ua": "python-httpx/0.28.1"}
```

**What is logged:** timestamp, HTTP method, path, status code, response time, user-agent.

**What is NOT logged:** IP addresses, search queries, airports, dates, results, or any user-identifiable data.
...
This server queries external provider APIs to retrieve pricing and availability data. Those providers have their own privacy policies. We send only the search parameters necessary to fulfill your query (e.g., airports, dates, preferences) -- never any personal or identifying information.
